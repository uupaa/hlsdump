...
function HLSDump(url,       // @arg URLString
                 options) { // @arg Object - { dir, autoStart, bulkDurations, readyCallback, updateCallback, endList, aac, mp4, pcm, ts, m3u8 }
                            ...
                            // @options.endList       Boolean = false - add #EXT-X-ENDLIST
                            // @options.m3u8          Boolean = false - dump media playlist
                            // @options.ts            Boolean = false - dump ts file
                            // @options.aac           Boolean = false - dump aac file
                            // @options.mp4           Boolean = false - dump mp4 file
                            // @options.pcm           Boolean = false - dump pcm file
    var that = this;

  //this._fs = global["require"] ? global["require"]("fs") : null;
    this._fs = null;
    ...
    this._playlistURL   = this._dir + "all.m3u8";
    ...
    this._endList       = options["endList"] || false;
    this._m3u8          = options["m3u8"]    || false;
    this._ts            = options["ts"]      || false;
    ...
    return new M3U8Spooler({
        ...
        "m3u8Callback": function(url, m3u8, playlist, master) {
            if (!master) {
                ...
                // MediaPlaylist のスナップショットをファイルに保存する
                var tsID = playlist["mediaSegments"][0]["tsID"];
                if (that._fs && that._m3u8) {
                    that._fs["writeFile"](that._dir + tsID + ".m3u8", m3u8); // 既に同じファイルがあれば上書きされる
                }
                // HLSDump_save で全てのMediaSegmentを含む playlist を作成するため MediaPlaylist を一時保存しておく
...
function HLSDump_stop() {
    this._spooler["stop"]();
    _writeM3U8.call(this);
}

function _writeM3U8() {
    if (this._fs) {
        // 全ての mediaSegments を含む完全な mediaPlaylist を VOD または Live 形式で作成する
        var lines = [
            "#EXTM3U",
            "#EXT-X-VERSION:3",
            "#EXT-X-ALLOW-CACHE:NO",
            "#EXT-X-TARGETDURATION:" + this._mediaPlaylistArray[0]["targetDuration"],
        ...
        for (var i = 0, iz = this._mediaPlaylistArray.length; i < iz; ++i) {
            ...
            for (var j = 0, jz = mediaSegments.length; j < jz; ++j) {
                ...
                if (!(tsID in tsIDMap)) {
                    lines.push("#EXTINF:" + tsDuration + ",", tsID + ".ts");
                    tsIDMap[tsID] = 1;
                }
            }
        }
...
function _demux(cachedDurations) {
    ...
    _toUint8Array(chunk["tsBlobs"], function(mpeg2ts_u8a) { // BlobArray to Uint8Array -> async
        ...
        if (that._fs) {
            _writeM3U8.call(that);
        }
        if (that._fs && that._ts) {
