...
function HLSDump(url,       // @arg URLString
                 options) { // @arg Object - { dir, autoStart, bulkDurations, readyCallback, updateCallback, live, aac, mp4, pcm, ts }
...
function _writeCombinedM3U8() {
    if (this._fs) {
        ...
        // Liveでは独自タグの #EXT-X-DUMP:YES を追加する
        var lines = [
            "#EXTM3U",
            "#EXT-X-VERSION:3",
        ...
        for (var i = 0, iz = this._mediaPlaylistArray.length; i < iz; ++i) {
            ...
            for (var j = 0, jz = mediaSegments.length; j < jz; ++j) {
                ...
                if (!(tsID in tsIDMap)) {
                    lines.push("#EXTINF:" + (tsDuration / 1000) + ",", tsID + ".ts");
                    tsIDMap[tsID] = 1;
                }
            }
        }
